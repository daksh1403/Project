...
public:
    ...
    void display() const {
        cout << left << setw(15) << regNo 
             << setw(25) << name 
             << setw(10) << getTotalPresent() << "/" << getTotalClasses()
             << setw(15) << fixed << setprecision(2) << calculateAttendancePercentage() << "%"
             << endl;
    }
...
public:
    ...
    bool addStudent(const string& regNo, const string& name) {
        if (isDuplicate(regNo)) {
            cout << "\n[ERROR] Student with RegNo '" << regNo << "' already exists!\n";
            return false;
        }
    ...
    void reportAttendancePercentage() const {
        ...
        for (const auto& student : students) {
            cout << left << setw(15) << student.getRegNo() 
                 ...
                 << setw(15) << fixed << setprecision(2) << student.calculateAttendancePercentage() << "%"
